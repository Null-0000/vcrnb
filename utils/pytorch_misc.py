...
from allennlp.training.trainer import move_optimizer_to_cuda    # allennlp0.8
# from allennlp.training.util import move_optimizer_to_cuda  # allenlp 1.2
...
def pad_sequence(sequence, lengths):
    ...
    for i, diff in enumerate(lengths):
        if diff > 0:
            output[i, 1:diff] = sequence[start:(start + diff-1)]
        start += diff
    return output
...
def save_checkpoint(model, optimizer, serialization_dir, epoch, val_metric_per_epoch, is_best=None,
                    learning_rate_scheduler=None) -> None:   # extras?
    """
