...
# from allennlp.data import Batch         # ALLENNLP1.2独有
...
class VCR(Dataset):
    def __init__(self, split, only_use_relevant_dets=True, add_image_as_a_box=True, embs_to_load='bert_da',
        ...
        self.h5fn_image = os.path.join(VCR_ANNOTS_DIR, self.split, f'image_feature_{self.split}.h5')
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(self.h5fn_image, 'r') as h5_features:
            # pytoch1.1
            group_image = {k: np.array(v) for k,v in h5_features[item['img_fn']].items()}
            image_feature = group_image['features']
            h5_label = group_image['labels'].tolist()
            assert len(h5_label) == image_feature.shape[0]
            # # pytoch1.6
            # h5_label = h5_features[item['img_fn']]['labels'][()].tolist()
            # image_feature = h5_features[item['img_fn']]['features'][()]
            # assert len(h5_label) == image_feature.shape[0]
...
def collate_fn(data, to_gpu=False):
    ...
    # # ALLENNLP1.2
    # if 'question_answer' in td:
    #     td['question_answer_mask'] = get_text_field_mask({'question_answer': td['question_answer']}, num_wrapping_dims=1)
    # if 'question_rationale' in td:
    #     td['question_rationale_mask'] = get_text_field_mask({'question_rationale': td['question_rationale']}, num_wrapping_dims=1)
    # td['answer_mask'] = get_text_field_mask({'answers': td['answers']}, num_wrapping_dims=1)
    # td['rationale_mask'] = get_text_field_mask({'rationales': td['rationales']}, num_wrapping_dims=1)

    # pytorch1.1
    td['box_mask'] = torch.all(td['boxes'] >= 0, -1).long()
    td['box_mask'][:, :1] = 0
    # # pytorch1.6
    # td['box_mask'] = torch.all(td['boxes'] >= 0, -1)
    # td['box_mask'][:, 0] = False
